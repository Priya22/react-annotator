...
def get_disagreements(data):
	...
	if not os.path.isfile(os.path.join('./temp', 'mention_log.txt')):
		mode = 'w'
	else:
		mode = 'a'
	
	log_file = open(os.path.join('./temp', 'mention_log.txt'), mode)
	pkl.dump(ann_data, open('./temp/'+title+'_mention_data.pkl', 'wb'))
	...
	print("Mention count: ", len(r_disagreements), file=log_file)
	
	print("Total ratio: ", len(r_disagreements)/len(r_to_text), file=log_file)

	return r_to_text, r_to_speakees, unmatched, r_disagreements
	...
	# #all disagreements
	# print("Writing mention disagreements: ")
	# file_name = title + '_mention_disagreements.txt'
	# file_path = os.path.join('./temp', file_name)
	# with open(file_path, 'w') as f:
	# 	print("\t\t\t MENTION DISAGREEMENTS \t\t\t", file=f)
	# 	print("--"*100, file=f)
	# 	print("\n", file=f)
